...
class Scenario:
    ...
    def fit_5(self):
        ...
        self.chunk_mean = int(np.floor((self.bandwidth_wireless * self.trace_length * self.result_mean) / self.size_of_chunk))
        self.chunk_std = ((self.bandwidth_wireless * self.trace_length * self.result_std) / self.size_of_chunk) ** 2
    ...
    def __init__(self, size_of_file=8000, size_of_chunk=80, bandwidth_wireless=40, trace_length=1000, mean=28, var=3.8, begin=0.001, end=0.08, resolution=1000):
...
class MAP:
    ...
    def algrithm_1(self):
        ...
        for k in range(self.total_chunk):
            ...
            EN_order = np.argsort(-self.phi_i_k[self.EN_start:, k])
...
example_scenario = Scenario(trace_length=800, size_of_file=40000, mean=24.488, var=3.81)
...
for i in range(example_MAP.total_EN):
    plot(x1, example_MAP.get_phi_i_k(i), color='k', linewidth=1)
plot(x1, example_MAP.get_prob_of_k(), color='k', linewidth=2)

plot([0, example_scenario.chunk_number], [example_MAP.threshold, example_MAP.threshold], color='k', linewidth=1, linestyle='--')
...
#plot([upward_spike, upward_spike], [0, y3[0]], color='red', linestyle=':')
xlabel('chunk number')
ylabel('probability')
show()
