...
class ACBC:
    ...
    def get_summery_of_deployment(self, return_value=NaN, print_result=False):
        ...
        if print_result == True:
            ...
            #print('PoF = ' + str(example_ACBC.get_price_of_fog()))
...
def generate_random_chunk(example_ACBC):
    # Feedback structure.
    # Select feedback EN, which is the EN have the most chunk cached.
    print('Generating random chunk......')
    feedback_EN = int(example_ACBC.get_summery_of_deployment(return_value='PeakSpot'))
    ...
    for item in example_ACBC.dictionary_of_EN.items():
    ...
    for i in range(len(feedback_chunks)):
        probability_list[i] = example_ACBC.get_phi_i_k(feedback_EN)[int(feedback_chunks[i])]
...
example_ACBC = ACBC(total_chunk=example_scenario.chunk_number,
                  ...
                  EN_start=0,
                  chunk_start=0)
example_ACBC.get_summery_of_deployment(print_result=True)

# Entire solution buffers:
# Include a dictionary that map the chunks and the ENs that cache these chunks
whole_dictionary = {key: '' for key in range(example_ACBC.total_chunk)}
for item in example_ACBC.get_dictionary().items():
    if item[1] != '':
...
whole_prob_of_k = zeros(example_ACBC.total_chunk)
for i in range(example_ACBC.total_chunk):
    if example_ACBC.get_prob_of_k()[i] != 0:
        whole_prob_of_k[i] = example_ACBC.get_prob_of_k()[i]
# Include an array of dictionary_array_str to save all dictionary_array_str
whole_dictionary_array_str = zeros(0)
whole_dictionary_array_str = append(whole_dictionary_array_str, example_ACBC.dictionary_array_str)


color_arg = 1
color_label = '-'
def change_color(color_arg):
    color_arg *= -1
    if color_arg == -1:
        color_label = '-'
    else:
        color_label = '--'
    return color_label, color_arg


end_chunk = 0
feedback_EN = 0
# If not finished, run the MAP over and over again
while(int(example_ACBC.last_chunk) != int(example_ACBC.total_chunk)):
    #int(example_ACBC.last_EN) != int(example_ACBC.total_EN-1)

    print('Run again...')
    end_chunk, feedback_EN = generate_random_chunk(example_ACBC)
    print(end_chunk, feedback_EN)
    end_chunk = example_ACBC.get_summery_of_deployment(return_value='MostFrequencychunkNumber')
    # Plot run again place (chunk number)
    plot([end_chunk + 1, end_chunk + 1],
         [0, 1],
         color='k',
         linewidth=1,
         linestyle=':')
    color_label, color_arg = change_color(color_arg)
    for i in range(example_ACBC.EN_start, int(feedback_EN+1)):
        plot(arange(example_ACBC.total_chunk),
             example_ACBC.phi_i_k[i],
             linestyle=color_label,
             color='k')
    print('end_chunk='+str(end_chunk)+'; feedback_EN='+str(feedback_EN))
    if feedback_EN+1 >= example_ACBC.total_EN:
        break
    example_ACBC = ACBC(total_chunk=example_scenario.chunk_number,
                      expectation_Xi=example_scenario.chunk_mean,
                      standard_deviation_Xi=example_scenario.chunk_std,
    ...
    print(example_ACBC.get_summery_of_deployment(print_result=True))
...
# plot the last time run result(phi_i_k)
color_label, color_arg = change_color(color_arg)
for i in range(example_ACBC.EN_start, int(example_ACBC.last_EN)+1):
    plot(arange(example_ACBC.total_chunk),
         example_ACBC.phi_i_k[i],
         linestyle=color_label,
         color='k')
...
print(counter)
print('Total PoF = '
      + str(counter/example_ACBC.total_chunk))

"""
# Output result dictionary to a txt file
text_value = open("value.txt", 'w')
for key, value in example_ACBC.get_dictionary().items():
    if value == '':
        text_value.write('-')
    else:
        text_value.write(value)
    text_value.write('\n')
...
# plot the graph
x1 = np.arange(example_ACBC.total_chunk)
#for i in range(example_ACBC.total_EN - example_ACBC.EN_start):
#    plot(x1, example_ACBC.get_phi_i_k(i))
plot(x1, example_ACBC.get_phi_i_k(1))
plot(x1, whole_prob_of_k, color='k', linewidth=1.5)


# threshold line
plot([0, example_scenario.chunk_number],
     [example_ACBC.threshold, example_ACBC.threshold],
     color='k',
     linewidth=1,
     linestyle='-.')
