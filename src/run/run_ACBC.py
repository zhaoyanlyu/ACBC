...
def run_ACBC(dict_env, env, tau, tot_user=1, num_thread=1):
    ...
    def one_user_fn(user_id):
        ...
        # ACBC algorithm
        while(True):
            map = MAP(tau=tau, tot_chunk=env.tot_chunk-chunk_start, tot_EN=dict_env['tot_EN']-EN_start,
                      mu_xi=env.chunk_mu, sigma_xi=env.chunk_sigma, EN_start=0)
            FEN = map.get_FEN()
            for index_EN in range(dict_env['tot_EN']-EN_start):
...
if __name__ == '__main__':
    ...
    for idx_test, speed_mu in enumerate(range(15, 34)):
        ...
        avg_hit, avg_miss, avg_PoF,\
        avg_num_feedback, finish_code_list = run_ACBC(dict_env=dict_env, env=env, tau=tau,
                                                      tot_user=dict_env['tot_user'], num_thread=50)

        result_dict = {'algorithm': 'ACBC'}
